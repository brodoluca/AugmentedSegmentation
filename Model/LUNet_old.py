...
class PointEncoder(nn.Module):
    ...
    def forward(self, points, neighbours):
        ...
        if(self.concat_channel == config.FOR_REFLECTANCE):
            torch.cat(( out, points[:, 0:4,:, :]), dim = 1)
        else:
            temp = points[:, :,0:3, :]
...
if __name__ == "__main__":
    points = Variable(torch.rand(32,5,1,config.IMAGE_HEIGHT*config.IMAGE_WIDTH))
    neighbours = Variable(torch.rand(32,5,8,config.IMAGE_HEIGHT*config.IMAGE_WIDTH))
